...
const form = useForm({
    ...
    quantidade: props.rebanho.quantidade,
    finalidade: props.rebanho.finalidade || '',
    data_atualizacao: props.rebanho.data_atualizacao ? new Date(props.rebanho.data_atualizacao) : new Date(),
    propriedade_id: props.rebanho.propriedade_id,
});

// Submeter formulário
const submit = () => {
    form.put(`/rebanhos/${props.rebanho.id}`, {
        onSuccess: () => {
            toast.add({
                severity: 'success',
                summary: 'Sucesso',
                detail: 'Rebanho atualizado com sucesso',
                life: 3000,
            });
        },
        onError: () => {
...
<template>
    <MainLayout>
        ...
        <div class="mb-4">
            <div>
                <h1 class="text-3xl font-bold text-900">Editar Rebanho</h1>
                <p class="text-sm text-600 mt-2">Atualize os dados do rebanho de {{ rebanho.especie }}</p>
            </div>
        </div>
