...
const STORAGE_KEY = `produtor-edit-tab-${props.produtor.data.id}`;
...
// Acessar os dados dentro da propriedade 'data'
...
const form = useForm({
    ...
    data_cadastro: parseDate(produtorData.data_cadastro_iso),
});

// Submeter formulário
// Nota: Toast é gerenciado automaticamente pelo MainLayout via flash messages
const submit = () => {
    form.put(`/produtores-rurais/${produtorData.id}`);
};
...
<template>
    <MainLayout>
        <Head title="Editar Produtor Rural" />

        <div class="mb-4">
            <div>
                <h1 class="text-3xl font-bold text-900">Editar Produtor Rural</h1>
                <p class="text-sm text-600 mt-2">Atualize os dados do produtor {{ produtorData.nome }}</p>
            </div>
        </div>

        <TabView v-model:activeIndex="activeTab">
            <!-- Aba de Dados do Produtor -->
            <TabPanel header="Dados do Produtor">
                <Card>
                    <template #content>
                ...
                <div class="documentos-container">
                    <div class="documentos-grid">
                        ...
                        <div class="upload-section">
                            <DocumentoUpload tipo="produtor" :id="produtorData.id" />
                        </div>
                        
                        <!-- Lista de documentos existentes -->
                        <div class="lista-section">
                            <DocumentoLista :documentos="produtorData.documentos || []" />
                        </div>
                    </div>
                </div>
            </TabPanel>
