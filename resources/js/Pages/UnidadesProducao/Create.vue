...
const form = useForm({
    nome_cultura: '',
    area_total_ha: null as number | null,
    coordenadas_geograficas: '',
    propriedade_id: null as string | null,
});

// Submeter formulário
// Nota: Toast é gerenciado automaticamente pelo MainLayout via flash messages
const submit = () => {
    form.post('/unidades-producao');
};
...
<template>
    <MainLayout>
        <Head title="Cadastrar Unidade de Produção" />

        <div class="mb-4">
            <div>
                <h1 class="text-3xl font-bold text-900">Cadastrar Unidade de Produção</h1>
                <p class="text-sm text-600 mt-2">Preencha os dados para cadastrar uma nova unidade de produção</p>
            </div>
        </div>
